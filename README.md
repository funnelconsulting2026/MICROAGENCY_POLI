...
| File | Cosa è |
|------|--------|
| `desktop.json` | Export Figma REST API del frame `REVISIONE UX/UI — DESKTOP_v2 · GEIST` (1416 × 6629) |
| `mobile.json` | Export Figma REST API del frame `REVISIONE UX/UI — MOBILE · GEIST` (390 × 8454) |
| `assets/` | Immagini del template esportate da Figma, nominate per node id (es. `192_138.png`). Se la cartella non c'è, vedi §7. |

I due JSON sono **read-only**. Non vanno mai modificati, riscritti o rigenerati. Sono la specifica visiva di partenza, non un file di lavoro.
...
**Regola di contesto.** I JSON si leggono solo alla fase 1. Dalla fase 2 in poi la stella polare è il codice generato, non il JSON. Non rileggere `desktop.json` o `mobile.json` per applicare una modifica del cliente: serve solo a bruciare contesto e a reintrodurre valori che sono già stati tradotti.
...
### Testi
Il contenuto sta in `characters`. Lo stile sta in `style`:
...
### Layout
`absoluteBoundingBox` dà `{x, y, width, height}` in coordinate assolute del canvas. Per ricavare la posizione relativa a un contenitore, sottrai le coordinate del padre.
...
`imageRef` è un hash, non un URL, e non contiene pixel. Vedi §7.
...
**Se `assets/` non esiste**: genera il template con placeholder neutri che rispettano esattamente le proporzioni e le dimensioni del contenitore originale. Nessun servizio esterno di immagini finte, nessuna immagine generata. Un blocco di colore piatto con l'alt text corretto.
...
## 9. Inventario sezioni (mobile, verificato)

Ordine reale nel frame mobile, con altezze originali. Serve per validare che la generazione non abbia perso o invertito blocchi.
...
La versione desktop contiene le stesse sezioni nello stesso ordine, con layout diverso. Il breakpoint di passaggio va impostato a 768px.
